...
def create_maze_env(load, env_name=None, seed=0):
  maze_id = None
  if env_name.endswith('Maze'):
    if load:
      maze_id = 'MazeTransfer'
      if env_name.startswith('Point'):
        return PointMazeEnv(maze_id=maze_id, maze_size_scaling=4, seed=seed, manual_collision=True)
